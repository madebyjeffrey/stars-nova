...
namespace Nova.WinForms.Gui
{
    ...
    public partial class NovaGUI : Form
    {
...

        {
            ...
            timerNextTurn.Interval = 1000;
            timerNextTurn.Tick += new EventHandler(timerCursor_Tick);
            timerNextTurn.Start();
            // 
            // timerNextTurn
            // 
        ...
        private readonly Point[] cursorShape =
{
        ...
        private void timerCursor_Tick(object sender, EventArgs e)
        {
            ...
            if ((!mapControl.isDrawing) && (!mapControl.isScreenRedrawing))
            {
                mapControl.isCursorCall = true;
                mapControl.MapPanel.Invalidate(mapControl.cursorRegion, false);
                mapControl.MapPanel.Update();
                mapControl.MapPanel.BackColor = Color.White;
                mapControl.MapPanel.BackColor = Color.Red;
                //mapControl.MapPanel.Invalidate();
                //mapControl.MapPanel.Refresh();
                mapControl.isCursorCall = false;
            }
        }
