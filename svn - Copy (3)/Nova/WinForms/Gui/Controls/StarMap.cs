...
namespace Nova.WinForms.Gui
{
    ...
    public partial class StarMap : UserControl
    {
        ...
        private readonly byte[] pathPoints =
        {
        ...
        public bool isDrawing = true; //Flag to indicate if StarMap.MapPanel is being modified at this moment
        private Brush CursorColor = Brushes.Yellow;
        public Region cursorRegion = null;
        ...
        public bool isCursorCall = false;
        public bool isScreenRedrawing = false;
    ...
    private void MapPanel_Paint(object sender, PaintEventArgs e)
        {
            isScreenRedrawing = true;
            e.Graphics.SmoothingMode = SmoothingMode.AntiAlias;
            if (isCursorCall)
            {
                if (CursorColor == Brushes.Yellow) CursorColor = Brushes.Red; else CursorColor = Brushes.Yellow;
                e.Graphics.FillRegion(CursorColor, cursorRegion);
            }
            else DrawEverything(e.Graphics);
            isScreenRedrawing = false;
        }
        ...
        private void DrawEverything(Graphics g)
        {
            ...
            // (6) Cursor.
            // cursorRegion is a copy of the region occupied by the cursor, it is used to cycle the cursor color
            NovaPoint position = LogicalToDevice(this.cursorPosition);
            position.Y += 5;
            cursorRegion = new Region(new GraphicsPath(cursorShape,pathPoints));
            Matrix translate = new Matrix();
            g.TranslateTransform(position.X, position.Y);
            //translate.Translate(position.X, position.Y);
            g.RotateTransform(180f);
            //translate.Rotate(180f);
            //g.FillPolygon(Brushes.Yellow, cursorShape);
            //cursorRegion.Transform(translate);
            g.FillRegion(Brushes.Yellow, cursorRegion);
            g.ResetTransform();
