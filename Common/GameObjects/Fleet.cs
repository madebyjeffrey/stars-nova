    ...
    public class Fleet : Mappable
    {
        ...
        public int MassDriverspeed
        {
            get
            {
                ...
                foreach (HullModule component in tokens.Values.First().Design.Hull.Modules) if ((component.AllocatedComponent != null) &&(component.AllocatedComponent.Properties.ContainsKey("Mass Driver"))) 
                    {
        ...
        public TravelStatus Move(ref double availableTime, Race race, out List<Message> messageOut, int targetVelocity, NovaPoint targetVelocityVector)
        {
            ...
            // to free warp speed.
            if ((arrived == TravelStatus.InTransit) && (targetTime > fuelTime) && (fuelConsumptionRate > this.FuelAvailable)) //dropping back to FreeWarp a little before completely running out of fuel can be beneficial as the player still has a little fuel and may be able to reach the destination by using a lower warpspeed (maybe)
            {
